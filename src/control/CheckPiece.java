...
public class CheckPiece {
    ...
    public Map<Piece, PieceStatus> getBuffer() {
        return buffer;
    }
    ...
    public CheckPiece(CheckPiece cp) {
        board = new Board();
        utils.copyBFromA(cp.board,this.board);
        this.buffer = new HashMap<Piece, PieceStatus>();
        utils.copyBFromA(cp.buffer,this.buffer);
        this.buffer = cp.buffer;
        this.count = cp.count;
        this.koX = cp.koX;
        this.koY = cp.koY;
    }
    ...
    public boolean check(int x, int y, int chessman) {
        ...
        if (y < 9 && board.getcell(x, y + 1) == ochers) {
        ...
        if (x < 9 && board.getcell(x + 1, y) == ochers) {
        ...
        if (count == 1) {
            this.board.setKo(koX,koY);
            count = 0;
        } else {
            this.board.setKo(-1,-1);
        }
        return true;
    }
     ...
     * @return boolean
     */
    public boolean isAlive(Piece piece, Direction dir) {
